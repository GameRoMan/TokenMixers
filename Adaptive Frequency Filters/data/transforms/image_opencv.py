...
# This file is for compatibility with affnet_v0.1. In future, we won't maintain it. Please use functions from
# image_pil.py
...
# add by huangzp
@register_transformations(name="bit_plane", type="image")
class BitPlane(BaseTransformation):
        def __init__(self, opts, h, w):
            # min_size = getattr(opts, "image_augmentation.random_resize.min_size", 256)
            # max_size = getattr(opts, "image_augmentation.random_resize.max_size", 1024)
            # interpolation = getattr(
            #     opts, "image-augmentation.random_resize.interpolation", "bilinear"
            # )
            super(BitPlane, self).__init__(opts=opts)
            self.h = h
            self.w = w
            self.weight = np.int16(np.ones([h, w, 8, 3]))
            self.bias = np.int16(np.ones([h, w, 8, 3]))
            for i in range(8):
                self.weight[:,:,i,:] = self.weight[:,:,i,:] * (2**(7-i))
                self.bias[:,:,i,:] = self.bias[:,:,i,:] * (2**i)

        def __call__(self, data: Dict) -> Dict:
            img = data['image']
            new_img = (self.weight & img[:,:,None,:]) * self.bias
            new_img = new_img.reshape(self.h, self.w, 24)
            # h,w = img.shape[0], img.shape[1]
            # new_img = np.zeros((h,w,24))
            # for c in range(3):
            #     for i in range(h):
            #         for j in range(w):
            #             n = str(np.binary_repr(img[i,j,c],8))
            #             for k in range(8):
            #                 new_img[i,j,3*k+c] = n[k]
            # # TODO: to check it out
            data['image'] = new_img
            return data

        def __repr__(self):
            return "{}(bit plane 3 3 3 3 3 3 3 3 sum 24 plane)".format(
                self.__class__.__name__,
            )
